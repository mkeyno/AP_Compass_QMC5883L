...
//
bool Compass::init(){    return true;}

void Compass::set_offsets(uint8_t i, const Vector3f &offsets)
{
    // sanity check compass instance provided
    if (i < COMPASS_MAX_INSTANCES)_offset[i].set(offsets); 
...
{
    for (uint8_t i=0; i<COMPASS_MAX_INSTANCES; i++) {        save_offsets(i);    }
}
...
void Compass::save_motor_compensation()
{
    ...
    for (uint8_t k=0; k<COMPASS_MAX_INSTANCES; k++) {        _motor_compensation[k].save();    }
}
...
{
    if (save_to_eeprom)        _declination.set_and_save(radians);
	else     			       _declination.set(radians);
	
}

float Compass::get_declination() const{    return _declination.get();}
...
bool Compass::configured(uint8_t i)
{
    ...
    if (i > get_count())     return false; 
    // exit immediately if all offsets are zero
    if (get_offsets(i).length() == 0.0f)     return false; 
