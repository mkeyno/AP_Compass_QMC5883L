...
#define AP_COMPASS_TYPE_QMC5883L  0x06
...
public:
 ...
 virtual uint8_t get_count(void) const { return 1; }
    ...
    bool use_for_yaw(void) const {        return _healthy[0] && _use_for_yaw;    }
    ...
    void set_board_orientation(enum Rotation orientation) {        _board_orientation = orientation;    }
    ...
    uint8_t motor_compensation_type() const {        return _motor_comp_type;    }
	...
	{
        if(_motor_comp_type == AP_COMPASS_MOT_COMP_THROTTLE)      _thr_or_curr = thr_pct; 
    }
    ...
    void set_current(float amps) {
        if(_motor_comp_type == AP_COMPASS_MOT_COMP_CURRENT)    _thr_or_curr = amps; 
